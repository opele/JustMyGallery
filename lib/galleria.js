...
Galleria.prototype = {
    ...
    */
    ...
    splice : function() {
        ...
        window.setTimeout(function() {
            var deleted = protoArray.splice.apply( self._data, args );
			if (args.length > 2) {
				...
				var deletedThumbnails = self._thumbnails.splice(args[0], args[1]);
				
				for (var i = 0; i < deletedThumbnails.length; ++i) {
					deletedThumbnails[i].container.remove();
				}
					var thumbchunk = self._thumbnails.slice(0);
				
				for (var i = 0; i < self._thumbnails.length; ++i) {
					self._thumbnails[i].index = i;
					self._thumbnails[i].data.order = i;

				var thumb = self._thumbnails[i];
